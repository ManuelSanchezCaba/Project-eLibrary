...
router.post('/api/book', (req, res) => {
    let { titulo, contenido, autor, fecha_creacion, categoria, descripcion, cantidad_pagina } = req.body;
    let tempDate = new Date();
    let date = tempDate.getFullYear() + '-' + (tempDate.getMonth()+1) + '-' + tempDate.getDate();
    fecha_creacion = date;
    mysqlConnection.query("insert into book(titulo, contenido, autor, fecha_creacion, categoria, descripcion, cantidad_pagina) values ('" + titulo + "','" + contenido + "', '" + autor + "', '" + fecha_creacion + "', '" + categoria + "', '" + descripcion + "', '" + cantidad_pagina +  "')", (err, rows, fields) => {
          if(!err) {
